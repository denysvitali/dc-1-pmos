...
// 5x7 font, carried over byte for byte from the C dc1-ask: column-major,
// bit 0 = top scanline, ASCII 0x20..0x7E (classic 5x7 forms).
var font = [95][5]byte{
...
// glyph maps one byte to its 5 font columns; anything outside ASCII folds to
// blank, as the C version did.
...
// textW is the width of str at scale s: 5 pixel columns plus one blank one
// per glyph.
func textW(s int, str string) int {
	return len(str) * 6 * s
}
