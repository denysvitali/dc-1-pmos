...
| Sensors | Daylight publishes nothing; the mainboard carries a connector explicitly labelled Light Sensor | From this unit: MC3416 accelerometer (AP i2c6), ambient-light/proximity part at i2c1 `0x49` (MEMSic `mn29xxx` family; bus staged for AP bring-up, sensor unbound), AP-wired hall switch; no gyro or magnetometer — see Sensors row |
...
| USB host | 🚧 Packaged, unverified | Charging Type-C hub path keeps the tablet a power sink, DR_SWAPs the data role to host, and leaves the gadget bound with D+ disconnected; hardware boot/session pending. — [hw/usb.md](hw/usb.md) |
