...
**What landed 2026-08-22.** Three changes shipped without hardware proof, and two things were measured live. In the kernel (commit `a2c27ab3bff1`, pinned the same day at linux pkgrel=28): every LVTS die zone gains a 105 °C hot trip, and the two board NTC zones gain their first-ever trips plus 2 s polling — compile-checked only, not yet booted on hardware. In the device package (pkgrel=57): the speaker-labeling UCM2 profile plus a WirePlumber profile override, parse-validated on-device but never executed through UCM on hardware. And the ambient-light part at i2c1 `0x49` received an identification dossier — MEMSic-family `mn29xxx` per the board's own SCP firmware strings, exact part unverified, node still undeclared. Measured live the same day on the running pkgrel=27 build: microphone capture works over the codec's DMIC path (see Audio), and the 2026-08-20 cpufreq/LVTS-trips/GPU-cooling commits are all active (see Thermal). Checklists for the next hardware session close this page.
