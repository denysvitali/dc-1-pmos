...
## Current dmesg triage (2026-08-28, live kernel r50)
...
| **P2 desktop/USB fixes queued in kernel r52 + device r87; boot verification pending** | hub `activate --> -11`; missing FUSE, Landlock/BPF-LSM, uinput/uhid and Bluetooth BNEP/RFCOMM; absent core/sysrq/SYN-cookie sysctls; orientation's early Mutter traceback; invalid WWAN key and unused ModemManager plugin ABI warnings | Kernel `05abbc2ae75c` corrects PIO MUSB's false DMA capability and enables the required runtime/sandbox features. Device r87 waits for Mutter/SensorProxy, preserves only valid WWAN policy, keeps ModemManager dormant on this modem-less board, and aligns packaged service/directory state. Close after a new boot plus repeated USB-hub reconnect shows the named signatures gone; the Lenovo 40B0 downstream-connect limitation remains a separate hardware compatibility item. |
